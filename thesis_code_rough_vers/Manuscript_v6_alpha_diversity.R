...
####nonIBD and CD colon (removed non-colonic strictures) ####
data <- nonIBD.CD.NI.colon
...
divnet.nonIBD.CD.colon.match.strictures <- divnet(data,
                                                  X= "DiseaseStatus",
                                                  ncores = 4)
saveRDS(divnet.nonIBD.CD.colon.match.strictures,"colon_nonIBDvCD_concordant_strictures.rds")
...
####CD tissue with discordant strictures removed####
data <- CD.NI.patients
data.concordant <- subset_samples(data,!(sample_data(data)$PatientNo %in% c("92","96")))
data.concordant <- prune_taxa(taxa_sums(data.concordant) > 0, data.concordant)
data <- data.concordant
...
data <- data.concordant
divnet.CD.concordant.tissue <- divnet(data,
                                      X= "AnatomSite",
                                      ncores = 4)
saveRDS(divnet.CD.concordant.tissue,"CD_tissue_concordant.rds")
...
data <- CD.NI.patients
data <- subset_samples(data,sample_data(data)$Behavior %in% c("B2"))
data <- prune_taxa(taxa_sums(data) > 0, data)
start.time.B2 <- Sys.time()
divnet.CD.B2 <- divnet(data,
                       X= "AnatomSite",
                       ncores = 4)
end.time.B2 <- Sys.time()
...
saveRDS(divnet.CD.B2,"CD_B2_tissue.rds")
...
saveRDS(divnet.CD.asa.colon,"CD_colon_5ASA_usage.rds")
